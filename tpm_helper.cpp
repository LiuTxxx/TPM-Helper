...
// int initialize_tpm() {
//     //- Init the context.
//     TSS2_TCTI_CONTEXT* tcti_ctx = tcti_device_init("/dev/...");
//     int rc = Esys_Initialize(&tpm_ctx, tcti_ctx, NULL);
//     if (rc != 0) {
...
int initialize_tpm_mssim() {
    ...
    //- Init the context.
    int rc = Esys_Initialize(&tpm_ctx, ctx, NULL);
    if (rc != 0) {
        LOG(ERROR) << "Esys_Initialize failed";
        return -1;
    } else {
        LOG(INFO) << "Esys_Initialize success";
    }
    return 0;
}
...
void get_performance(uint32_t nv_handle) {
    ...
    std::chrono::duration<double, std::milli> timer_span;
    std::vector<double> timer_res;
    ...
    for (i = 0; i < test_count; i++) {
        ...
        timer_span = timer_stop - timer_start;
        timer_res.push_back(timer_span.count());     
    ...
    LOG(INFO) << "The total elapsed = " << elapsed.count() << "ms" << " i = " << i;
    for (double tmp: timer_res) {
        // printf("%f\n", tmp);
        LOG(INFO) << tmp;
    }
    return ;
